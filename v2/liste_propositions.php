...
if ($result->num_rows == 1) {
    ...
    if ($result_propositions->num_rows > 0) {
        ...
        while ($row_proposition = $result_propositions->fetch_assoc()) {
            ...
            // Condition pour vérifier la valeur du verrou
            $verrou = $row_proposition["verrou"];
            $buttonStyle = ($verrou == 1) ? "style='color: red;'" : "";
        
            // Ajouter des boutons "Modifier" et "Supprimer" pour les propositions de l'utilisateur avec le style approprié
            
                echo "<form action='modifier_proposition.php' method='get'>";
                echo "<input type='hidden' name='id_pro' value='" . $row_proposition["id_pro"] . "'>";
                echo "<button type='submit' $buttonStyle>Modifier</button>";
                echo "</form>";
        
                echo "<form action='supprimer_proposition.php' method='get'>";
                echo "<input type='hidden' name='id_pro' value='" . $row_proposition["id_pro"] . "'>";
                echo "<button type='button' onclick='confirmDelete(" . $row_proposition["id_pro"] . ")'>Supprimer</button>";
                echo "</form>";
...
<script>
    function confirmDelete(id) {
        if (confirm("Êtes-vous sûr de vouloir supprimer cette proposition et toutes ses versions associées ?")) {
            window.location.href = 'supprimer_proposition.php?id_pro=' + id;
        }
    }
</script>
</html>
